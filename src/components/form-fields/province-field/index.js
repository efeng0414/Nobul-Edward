import React, { Component } from "react";
import PropTypes from "prop-types";
import { injectIntl, intlShape } from "react-intl";
import { Form, Select } from "antd";
import { bound } from "class-bind";
import { getFormLayout } from "../../../utilities/forms";
import { translate } from "../../../utilities/locale";
import { objectIsEmpty } from "../../../../core/utilities/misc";

import { PROVINCES } from "../../../../core/data/provinces";
import { DEFAULT_COUNTRY } from "../../../../core/constants/shared";

const FormItem = Form.Item;
const Option = Select.Option;
class ProvinceField extends Component {
  static propTypes = {
    form: PropTypes.any,
    required: PropTypes.bool,
    intl: intlShape.isRequired,
    country: PropTypes.string,
    onChange: PropTypes.func,
    name: PropTypes.string,
    layout: PropTypes.object,
    initialValue: PropTypes.string,
    labelKey: PropTypes.string,
    messageKey: PropTypes.string
  };

  static defaultProps = {
    form: {},
    required: false,
    onChange: () => {},
    country: DEFAULT_COUNTRY,
    name: "province",
    layout: {},
    initialValue: "",
    labelKey: "province",
    messageKey: "province"
  };

  @bound
  filterByCountry(country) {
    return PROVINCES.filter(
      province => (country ? country === province.country : true)
    );
  }

  render() {
    const fieldLayout = objectIsEmpty(this.props.layout)
      ? this.props.layout
      : getFormLayout();
    const provinces = this.filterByCountry(this.props.country);

    return (
      <FormItem
        {...fieldLayout}
        label={translate(this.props.intl, this.props.labelKey)}
        hasFeedback
      >
        {this.props.form.getFieldDecorator(this.props.name, {
          initialValue: this.props.initialValue,
          rules: [
            {
              required: this.props.required,
              message: translate(
                this.props.intl,
                `error.${this.props.messageKey}IsRequired`
              )
            }
          ]
        })(
          <Select
            showSearch
            optionFilterProp="children"
            filterOption={(input, option) =>
              option.props.children
                .toLowerCase()
                .indexOf(input.toLowerCase()) >= 0
            }
            onChange={this.props.onChange}
          >
            {provinces.map(({ code, label }) => (
              <Option key={code} value={code}>
                {label}
              </Option>
            ))}
          </Select>
        )}
      </FormItem>
    );
  }
}

export default injectIntl(ProvinceField);
